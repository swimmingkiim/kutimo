...
export type KutimoFunctionName = string;
 ...
 * Currently, fragment, strings, values, eventCallback are being used in WCSBaseElement(kutimo-element).
 */
export type KutimoHTMLCompileResult = {
  authorToken: string;
  validation: boolean;
  sanitized: boolean;
  ...
  eventCallback: {
    [eventName: string]: {
      [name: KutimoFunctionName]: Function;
    }
  };
}

/**
 * Stamp string that result is compiled from kutimo-html.
 */
 ...
 * @return {KutimoHTMLCompileResult}
 */
const initResult = (strings: TemplateStringsArray, ...values: unknown[]): KutimoHTMLCompileResult => {
  return {
...
/**
 * validateAuthor checks whether the WCSHTMLCompiledResult is made for kutimo-html.
 ...
 * @param result {KutimoHTMLCompileResult}
 * @param strings {TemplateStringsArray}
 * @param values {unknown[]}
 * @return {string}
 */
const concatHTMLStrings = (result: KutimoHTMLCompileResult, strings: TemplateStringsArray, ...values: unknown[]) => {
...
/**
 * html is main function of kutimo-html.
 * It takes template literal as parameter and returns WCSHTMLCompileResult.
 * @param strings {TemplateStringsArray}
 * @param values {unknown[]}
 * @return {KutimoHTMLCompileResult}
 */
export const html = (strings: TemplateStringsArray, ...values: unknown[]): KutimoHTMLCompileResult => {
